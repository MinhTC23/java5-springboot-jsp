...
<section>
    <div class="container">
        <h1>Capacity</h1>
        <form:form action="/capacity/add" method="post" modelAttribute="cp">
        ...
        <table class="table">
            ...
            <c:forEach items="${capacitys}" var="cp">
                <tr>
                    ...
                    <td>${cp.name}</td>
                    <td>${cp.dateCreate}</td>
                    <td>${cp.dateCorrect}</td>
                    <td>${cp.status == 1 ? "Hoạt động" : "Không hoạt động"}</td>
                    <td>
                        <a href="/capacity/view-update/${cp.id}" class="btn btn-warning" role="button">Update</a>
                        <a href="/capacity/delete/${cp.id}" class="btn btn-danger" role="button"
